...
void main (void) {
    ...
    float vd = dot(vw, -n);
    tex_coord1 = vec2(.5 + vd / cylinder_radius, 0.5);
...
void main()
{
    vec4 color = texture2D(texture0, vec2(1. - tex_coord0.x, tex_coord0.y));
    vec4 gradient = texture2D(texture1, tex_coord1);
    //vec4 gradient = vec4(tex_coord1.x, 0, 0, 1);
    //gl_FragColor = vec4(color.rgb*(1.0 - gradient.a) + gradient.rgb, color.a); // premultiplied alpha
    gl_FragColor = vec4(color.rgb * gradient.rgb * 1.35, color.a); // premultiplied alpha
    //gl_FragColor = color.rgba;
    //gl_FragColor = gradient.rgba;
}
